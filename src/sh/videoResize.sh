...
while (( END > 0 )) ; do
  ...
  if [[ "${videoSize}" != "${newVideoSize}" || ${forceVideoGeneration} ]] ; then
    ...
    $( set ${explainMode} ; ${ffmpegCommand} ${FFMPEG_VERBOSE_OPTIONS} ${ffmpegSampleOption} -i ${fileName} ${FFMPEG_ENCODING_QUALITY_OPTION} -vf "scale=${newVideoSize},setsar=1/1" ${FFMPEG_PRESERVE_METADATA_OPTION} ${newFileName})
    if [[ $? != 0 ]] ; then
