...
while (( ${#} > 0 )) ; do
  ...
  # check file Access
  checkFileAccess ${fullFileName} || continue

  [ "${useFileName4Description}" == "yes" ] &&  descriptionUpdate="-${TAG_DESCRIPTION}=$(basename ${fullFileName})"

  CR=$(set ${explainMode} ; exiftool ${exiftoolOptions} ${authorUpdate} ${creatorUpdate} ${descriptionUpdate} ${editorUpdate} ${publisherUpdate} ${titleUpdate} ${sourceUpdate} ${keywordsUpdate} ${extraUpdate[@]} ${fullFileName})
  if [[ $? != 0 ]] ; then
