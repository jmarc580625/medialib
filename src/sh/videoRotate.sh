...
while (( END > 0 )) ; do
  ...
  $( ${explainMode} ; ${ffmpegCommand} ${FFMPEG_VERBOSE_OPTIONS} ${ffmpegSampleOption} -i ${fileName} ${FFMPEG_ENCODING_QUALITY_OPTION} -vf ${filter} ${FFMPEG_PRESERVE_METADATA_OPTION} ${newFileName})
  if [[ $? != 0 ]] ; then
