...
# usage & help
#-------------------------------------------------------------------------------
readonly USAGE='usage: %s [-h] [-t] [-v] [-f] [-e] (-u | ((-r | -w) "<who> <what> <where>")) <source files>'
readonly HELP="
Rename video files according to the following pattern: <who>-<what>@<where>[<duration>,<size>(,G)?].<ext>
  ...
  -t: test mode
  -v: verbose mode
  -f: force mode, must be used in combination with -w
  -e: explain mode
  ...
  -w: rename the file according to the pattern
      ...
      no renaming is performed if the filename follows the pattern unless force
      mode is used
"
#-------------------------------------------------------------------------------
#  import section
...
readonly FILENAME_PATTERN='s/^[a-z][.a-z]*-[-.a-z0-9]*@[-.a-z]*\[[-,x0-9]*(,G)?\].*//'
declare exiftoolCommand=${EXIFTOOL_COMMAND}
declare explainMode=":"
declare forceMode=0

# parse options
while getopts ":h :v :t :e :f :u :r: :w:" opt; do
  case ${opt} in
    h)
      help
      exit 0
...
while (( END > 0 )) ; do
  ...
  case ${action} in
    ${ACTION_CREATE})
      if [[ "${fileNamePatternMatching}" == "" ]] && ! (( ${forceMode} )) ; then
  ...
  CR=$(${explainMode} ; exiftool "-filename<${fileCategory}"'[${duration;s/:/-/g;s/([0-9][0-9])(\.[0-9]*)( s)/0-00-$1/;s/([0-9])(\.[0-9]*)( s)/0-00-0$1/},${Imagesize}'${geoTag}']%-c.%e' ${fullFileName})
  if [[ $? != 0 ]] ; then
