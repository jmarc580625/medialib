...
while (( END > 0 )) ; do
  ...
  $(set ${explainMode} ; ${ffmpegCommand} ${FFMPEG_VERBOSE_OPTIONS} ${ffmpegSampleOption} -i ${fileName} ${FFMPEG_ENCODING_QUALITY_OPTION} -vf ${cropSpec} ${FFMPEG_PRESERVE_METADATA_OPTION} ${outFileName})
  if [[ $? != 0 ]] ; then
