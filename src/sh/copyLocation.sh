...
readonly HELP="
Rename video files according to the following pattern: <who>-<what>@<where>[<duration>,<size>].<ext>
  -h: display this help
...
readonly FILENAME_PATTERN='s/^[a-z]*-[-.a-z0-9]*@[-.a-z]*\[[-,x0-9]*\].*//'
...
while (( END > 0 )) ; do
  ...
  fileNamePatternMatching=$(echo ${fileNameWithoutExtention} | sed -e ${FILENAME_PATTERN})
  ...
  CR=$(set ${explainMode} ; exiftool "-filename<${fileCategory}"'[${duration;s/:/-/g;s/([0-9][0-9])(\.[0-9]*)( s)/0-00-$1/;s/([0-9])(\.[0-9]*)( s)/0-00-0$1/},${Imagesize}]%-c.%e' ${fullFileName})
  if [[ $? != 0 ]] ; then
