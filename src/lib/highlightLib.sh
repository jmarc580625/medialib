...
# parameters
#   $1: highlighter function name
#   $2: custom highlighting function name, default function is provided when empty
#   $3: file where highlighter function writes, default to stdout when empty
#   S4: forces highliting function 
# outcome
#   Generates an highlighter function with provided name
#   highlighter function recieves text as input parameter and writes it on the specified output file 
#   text is highlighted with highlighting function only when writing on terminal unless force mode is on
  declare name=$1
  ensure [[ ! -z ${name} ]]
