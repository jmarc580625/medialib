...
readonly _OUT_FORMAT_DEFAULT=${_OUT_FORMAT_JSON}
...
# functionRC=$1  ; # expectedFunctionRC=$2  ; functionName=$3 ; functionParameters=$4+
function reportFunctionRC  {
  ensure isInteger $1;  local toTestRC=$( (( $1 == 0 )) ; echo $?)
  ensure isInteger $2;  local expectRC=$( (( $2 == 0 )) ; echo $?)
  controlRC=$( ((toTestRC == expectRC)) ; echo $? )
  local functionUnderTest=${3}
  local testResult=$( [[ ${controlRC} == 0 ]]  && echo ${TEST_PASS} || echo ${TEST_FAIL} )
  local controlerReport="expect('RC=${toTestRC}' == '${expectRC}')=>${controlRC}" ; traceVar controlerReport
  controlerReport=${controlerReport/=>0/=>${CONTROL_OK}} ; controlerReport=${controlerReport/=>1/=>${CONTROL_KO}} ; traceVar controlerReport
  local functionReport="${functionUnderTest}$(_formatParams "${@:4}")=>${toTestResult}" ; traceVar functionReport
  local toTestResult=""
...
# expectedFunctionRC=$1  ; controledFunctionVector=$2
function controlFunctionRC  {
  ensure isInteger $1;  local expectRC=$( (( $1 == 0 )) && echo 0 || echo 1)
  if isBlank $2 ; then
    toTest="" ; traceVar toTest
  else
    ensure isArray $2; declare -n functionArray=$2 ; traceVar functionArray
  fi
  ...
  toTestResult=$(${toTest} "${params[@]}")
  toTestRC=$( (( $? == 0 )) ; echo $? )
  controlRC=$( ((toTestRC == expectRC)) ; echo $? )
  local testResult=$( [[ ${controlRC} == 0 ]]  && echo ${TEST_PASS} || echo ${TEST_FAIL} )
  OFS=$IFS; IFS=$'\n' controlReport=($(xargs -n1 <<<${toTestResult})) ; IFS=$OFS
  local functionReport="${toTest}$(_formatParams "${params[@]}")=>${toTestResult}" ; traceVar functionReport
  local controlerReport="expect('RC=${toTestRC}' == '${expectRC}')=>${controlRC}" ; traceVar controlerReport
  controlerReport=${controlerReport/=>0/=>${CONTROL_OK}} ; controlerReport=${controlerReport/=>1/=>${CONTROL_KO}} ; traceVar controlerReport
