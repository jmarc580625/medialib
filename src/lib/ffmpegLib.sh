...
readonly FFMPEG_HIDEBANNER_OPTIONS=' -hide_banner '               ; traceVar FFMPEG_HIDEBANNER_OPTIONS
readonly FFMPEG_VERBOSE_OPTIONS=' -v warning '                    ; traceVar FFMPEG_VERBOSE_OPTIONS
...
declare BLACK_DETECT_THREASHOLD=0.60
declare DURATION_SEARCH=10

function blackIntroDetect() {
  local fileName=$1
  ffmpeg                          \
    ${FFMPEG_HIDEBANNER_OPTIONS}  \
    -to ${DURATION_SEARCH}        \
    -i ${fileName}                \
    -vf "blackdetect=d=0.05:pix_th=${BLACK_DETECT_THREASHOLD}" \
    -an                           \
    -f null - 2>&1                |
      grep black_start:0          |
         sed -e 's/^.*_end://' -e 's/ .*$//'
...
function getNextIFrameIndex() {
  ...
  LC_ALL_SAVE=${LC_ALL}; export LC_ALL=C
  ffprobe                                                           \
