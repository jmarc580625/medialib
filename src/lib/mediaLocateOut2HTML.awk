...

{
  ...
  if ( truePosition ){
    ...
    cmd | getline hash
    close(cmd)
    thumbnailMediaFile = thumbDir "/" hash ".jpg"
    openStreetMapInfo =  thumbDir "/" hash ".html"
    exifInfo =           thumbDir "/" hash "-exif.html"
    ...
    if (mediaType == "MOVIE"){
      ...
      execCmd("convert -resize 128x \"" mediaFile "\" " thumbnailMediaFile)
    } else {
      mediaToShow = 0
    }
