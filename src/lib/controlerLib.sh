...
function _formatValue {
  if [[ "$@" =~ ^[[:space:]]*$ ]] ; then
    echo \'"$@"\'
  elif [[ $(declare -p "$@" 2>&-) =~ ^declare[[:space:]]-[aA] ]] ; then
    declare -p "$@" | sed -e "s/^declare -. [_[:alpha:]][_[:alpha:][:digit:]]*//" -e "s/=//" -e "s/\"/\'/g" -e "s/ )/)/" -e "s/^$/\(\)/"
  elif [[ $(declare -p "$@" 2>&-) =~ ^declare[[:space:]]- ]] ; then
    declare -n _v=$@; echo \'$_v\'
  else
    echo \'$@\' 
  fi
} 
...
#     [1] control criteria
# $@:2 function to control and its parameters
...
function control        {
  ...
  if isArray "$2" ; then
    ...
    valueDisplay="\"${toTest}$(_formatParams "${params[@]}")=>${valueToControl}\""
  ...
  local controlDisplay="\"${controler}$(_formatParams "${criteria}" "${valueToControl}")=>${controlResult}\""
