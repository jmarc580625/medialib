...
#-------------------------------------------------------------------------------
# Initialization
# check LIB_PATH is set before
if [[ -z ${LIB_PATH+x} ]] ; then
  echo FATAL ERROR:LIB_PATH must be set before importing testingHelper >&2
  exit 1
fi
...
function checkDir    {
  ...
  if [[ ! -d ${dirName} ]] ; then
    fatal "file '${dirName}' do not exist or not a directory"
  fi
  # check if the directory has read access
  if [[ ! -r ${dirName} ]] ; then
    fatal  "no read access to '${dirName}'"
  fi
  echo ${dirName}
}
function chekFiles    {
...
Execute test suite
  ...
  -i: execute test items in the list
      items id are enclosed between double quote and separated by space
  -l: display testing information
      test suite id
      list of test items
  -o: select test report formating style
      supported styles are xml, json and text
"
#-------------------------------------------------------------------------------
# parse options
[[ "${testingHelper_WithTrace}" != "true" ]] && enableTrace

while getopts ":h :v :a :l :i: :o:" opt; do
  case ${opt} in
    h)
      help
      exit 0
