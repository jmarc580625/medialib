...
#-------------------------------------------------------------------------------
# ensure proper awk version is installed
declare awkCmd=""
[[ -z ${awkCmd} ]] && gawk -W version | grep -qi gnu && awkCmd=gawk
[[ -z ${awkCmd} ]] && awk  -W version | grep -qi gnu && awkCmd=awk
[[ -z ${awkCmd} ]] && fatal "videoSizingLib requires GNU awk"
...
_aspectRatio[1x1]=$(${awkCmd} "BEGIN{print 1/1}")
_aspectRatio[5x4]=$(${awkCmd} "BEGIN{print 5/4}")
_aspectRatio[4x3]=$(${awkCmd} "BEGIN{print 4/3}")
_aspectRatio[16x10]=$(${awkCmd} "BEGIN{print 16/10}")
_aspectRatio[16x9]=$(${awkCmd} "BEGIN{print 16/9}")
...
function getVideoOrientation  {
  ...
  ${awkCmd} -v videoWidth=${videoWidth} \
...
function getVideoRatio        {
  read -r videoWidth videoHeight <<< $(_getWidthHeight "$@") ; traceVar videoWidth ; traceVar videoHeight
  ${awkCmd} -v videoWidth=${videoWidth} -v videoHeight=${videoHeight} \
    "BEGIN {
